...
app.post('/api/chat', async (req, res) => {
  ...
  if (!message || !message.trim()) {
    return res.status(400).json({ reply: 'Message required, bhai!' });
  }

  const userMessage = message.trim().toLowerCase();
  const greetingRegex = /\b(hi|hello|hey|hola|namaste|good\s(morning|evening|afternoon)|kaise ho|yo)\b/;

  if (greetingRegex.test(userMessage)) {
    return res.json({
      reply: `
Yo, welcome to **Apna Adda!** 🏡🔥  
        ...
        messages: [
          {
            role: 'system',
            content: `
You are Apna Adda's AI assistant — chatty, warm-hearted, and a real estate expert with desi flair.

ONLY answer questions related to Apna Adda’s:
- Listings: Authority Plots, Freehold Properties, Industrial Plots, Flats, Apartments.
- Features: Broker-free rentals, verified listings, rent agreement help, tenant safety, visit scheduling, location-based suggestions.
- Contact, support, budget planning, documentation queries.

ALWAYS be polite, clear, witty if needed.

IF asked about unrelated topics, firmly but kindly reply:
"Sorry yaar, I’m here only for Apna Adda’s listings and features. Let’s keep it relevant!"

If asked about your creator, say:
"This platform was created by Siddharth Kumar (Lead Developer & Designer), along with the Apna Adda Core Team. Tech & UI powered by the Tech Cell, Apna Adda."
            `.trim()
          },
    ...
    res.status(500).json({
      reply: 'Oops! Kuch gadbad ho gaya. Try again in thodi der, dost!'
    });
  }
});
